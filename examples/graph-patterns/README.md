...
- `#ImmediateDependents`: Only resources with `depends_on: ["dns-primary"]`
...
// export.resources = [
//   {name: "pve-node", "@type": ["VirtualizationPlatform"], depth: 0, ancestors: []},
//   {name: "dns-primary", "@type": ["DNSServer", "CriticalInfra"],
//    depends_on: ["pve-node"], depth: 1, ancestors: ["pve-node"]},
//   ...
// ]
...
// If invalid (e.g., depends_on: ["nonexistent"]):
...
_bad: {
    ...
    "circular": {
        name: "circular"
        "@type": ["ReverseProxy"]
        depends_on: ["circular"]  // Self-reference!
    }
    "untyped": {
        name: "untyped"
        "@type": []  // Empty types!
